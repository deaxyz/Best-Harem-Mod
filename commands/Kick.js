...
let reawEmbed = new Discord.MessageEmbed().setColor("f1f1f1").setFooter("Ravlex bebeğim çok mükemmelsin :)").setTimestamp().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true}))
let embed = reawEmbed;
...
message.guild.channels.cache.get(a.banLog).send(reawEmbed.setDescription(`${client.emojis.cache.get(a.yes)} ${member} kullanıcısı ${message.author} tarafından "${sebep}" sebebiyle atıldı!`));
